...
_.extend(dispatch.SpreadingDispatcher.prototype, {
    ...
    newPost: function(post) {
        ...
        _.each(this.usersToSpreadTo, function(user) {
            post.publishTo(user);
        }, this);
    },
    ...
    postVote: function(post, voter) {
        ...
        // then do a spread according to the on-vote-spread factor. For now,
        // we don't have a metric for triggering promotion so we'll set it
        // to false and just never promote.
        if(false) {
            this.promotePost(post);
        } else {
    ...
    getRandomUnsubscribedUsersFromPost: function(numUsers, post) {
        ...
        var validUsers = _.without(this.event.get("users").toArray(),
            post.get("usersSubscribed").toArray());
