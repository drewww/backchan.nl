...
_.extend(dispatch.SpreadingDispatcher.prototype, {
    ...
    newPost: function(post) {
        ...
        if(usersToSpreadTo.length<this.options["starting-spread"]) {
            // we have no one left to spread to. that means the post is
            // at maximum spread + everyone has seen it already.
            this.promotePost(post); 
        }
    ...
    postVote: function(post, voter) {
        ...
        if(this.event.get("posts").getPostRank(post) <
            ...
            if(usersToSpreadTo.length<this.options["on-vote-spread"]) {
                // we have no one left to spread to. that means the post is
                // at maximum spread + everyone has seen it already.
                this.promotePost(post); 
            }
    ...
    getRandomUnsubscribedUsersFromPost: function(numUsers, post) {
        ...
        // short circuit this process if they're asking for no users.
        // this happens for certain server/test configurations. 
        if(numUsers==0) {
            return [];
        }
        ...
        // catch a couple of cases where there's really no randomness to the
        // situation: dispatch is asking for more users than we actually have
        if(validUsers.length==numUsers) return validUsers;
        if(validUsers.length < numUsers) return validUsers;
