...
server.BackchannlServer.prototype = {
    ...
    reset: function(options) {
        ...
        // not doing anything with production quite yet, but will.
        _.defaults(options, {
            "production":false,
            "test-event":false,
            "dispatcher":"base",
        ...
        // these are kind of vestigal - persist is on all the time now.
        actions.journalingEnabled = true;
        server_model.setPersist(true);
        ...
        if(options["load"]) {
            var self = this;
            this.allUsers.fetch({success: function() {
                self.events.fetch({success: function() {
                    
                    options.callback && options.callback();
                }});
            }});
        } else {
            this.checkTestEvent(options);
            options.callback && options.callback();
        }
        
    },
    
    checkTestEvent: function(options) {
    ...
    start: function(host, port) {
        ...
        if(!this.production) {
            this.app.get('/test/', function(req, res) {
                res.render('test.ejs', {layout:false, locals:{"host":host,
                    "port":port}});
            });
        }
