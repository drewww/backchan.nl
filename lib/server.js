        ...
        this.io.sockets.on('connection', function(socket) {
            ...
            socket.on("join", function(data) {
                socket.get("identity", function(err, userId) {
                    logger.debug("eventId: " + data["eventId"]);
                    logger.debug("userID: "  +userId);
                    if(_.isNull(userId) || _.isUndefined(userId)) {
                        logger.debug("USERID WAS UNDEFINED");
                        socket.emit("join-err", {"err":"User not identified yet."});
                        return;
                    }
                    ...
                    if(!("eventId" in data)) {
                        socket.emit("join-err", {"err":"No eventId present"});
                        return;
                    }
                    logger.debug("eventId isNumber: " + _.isNumber(data["eventId"]));
                    
                    logger.debug("eventId in data: " + ("eventId" in data));

                    if(!(_.isNumber(data["eventId"]))) {
                        socket.emit("join-err", {"err":"Wrong datatype for eventId"});
                        return;
                    }
                    var e = socket.server.events.get(data["eventId"]);

                    logger.debug("eventId is legit: " + _.isNull(e));

                    if(_.isNull(e) || _.isUndefined(e)) {
                        socket.emit("join-err", {"err":"Invalid eventId."});
                        return;
                    }
