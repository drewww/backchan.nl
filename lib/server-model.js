...
server_model.ServerEvent = model.Event.extend({
    ...
    initialize: function(args) {
        ...
        if("journaling" in args && args["journaling"]) {
            ...
            this.get("journaler").registerNewModel(this);
        }
    },
    ...
    addPost: function(post) {
        ...
        if(!_.isNull(this.get("journaler"))) {
            this.get("journaler").registerNewModel(post);
        }
...
server_model.Journaler.prototype = {
    ...
    writeAttributes: function(entryType, model) {
        ...
        else setAttributes = model.attributes;
        
        var entry = {objectType:this.getType(model),
                     id: model.id,
                     timestamp:new Date().getTimestamp(),
                     entryType:entryType, 
