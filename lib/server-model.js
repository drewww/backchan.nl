...
server_model.ServerEvent = model.Event.extend({
    ...
    initialize: function(args) {
        ...
        if("journaling" in args && args["journaling"]) {
            this.set({
                "journaler":new server_model.Journaler(
                    "./db/events/" + this.id + ".journal")});
            
            this.get("journaler").registerModel(this);
        }
    },
    ...
    addPost: function(post) {
        ...
        if(!_.isNull(this.get("journaler"))) {
            this.get("journaler").registerModel(post);
        }
...
server_model.Journaler = function(path) {
...
server_model.Journaler.prototype = {
    baseDbDirectory: "../db/",
    file: null,
    running: false,
    
    registerModel: function(newModel) {
        if(!this.running) return;
        
        newModel.bind("change", this.handleModelChange, this);
        
        this.writeAttributes("CREATE", newModel);
    },
    
    handleModelChange: function(changedModel) {
        if(!this.running) return;
        
        this.writeAttributes("CHANGE", newModel)
    },
    
    writeAttributes: function(entryType, model) {
        // entryType is CREATE or CHANGE
        
        var setAttributes = null;
        
        // we might need to have a more formal way of doing transient
        // attributes to deal with this
        if(entryType=="CHANGE") setAttributes = model.changedAttributes;
        else setAttributes = model.toJSON();
        
        var entry = {objectType:this.getType(model),
                     id: model.id,
                     timestamp:new Date().getTime(),
                     entryType:entryType, 
                     attributes: setAttributes};
        
        // TODO BE CAREFUL HERE - using carriage returns to split means we
        // have to be sure that all the user-enterable text fields have been
        // sanitized to not have any carriage returns in them. 
        this.file.write(JSON.stringify(entry) + "\n");
    },
    
    getType: function(model) {
        if(model instanceof server_model.ServerPost) return "POST";
        else if(model instanceof server_model.ServerEvent) return "EVENT";
        else if(model instanceof server_model.ServerChat) return "CHAT";
        else return "UNKNOWN";
    },
    
    stopJournaling: function() {
        // flush the file, close the file pointer. This needs to be done on
        // shutdown, so we'll need to track all the open journalers somehow. 
        // I guess some global variable in server_model? deal with later.
        fs.close(this.file, function(err) {
