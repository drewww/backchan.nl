...
server_model.ServerPost = model.Post.extend({
    ...
    publishTo: function(user) {
        ...
        // make sure to remove the user from the subscribed user list
        // and remove it from the channel.
        user.bind("disconnected", function() {
            // console.log("disconnect triggering on post");
            usersSubscribed.remove(user);
            user.removeFromChannelForEvent(this.getChannel(),
                this.get("event"));
        }, this);
    ...
    publishToSocket: function(socket) {
        // This is to handle a subtle case: when someone connects to the same
        // event a second time, we need a way to just send posts to that 
        // socket, even though the user as a whole is already subscribed and
        // will get appropriate updates.
        socket.emit("post", JSON.stringify(this.toJSON()));
    },
    ...
    promote: function(promotedAt) {
        var didPromote = model.Post.prototype.promote.call(this, promotedAt);
...
server_model.ServerUser = model.User.extend({
    ...
    joinedEventWithSocket: function(event, socket) {
        ...
        event.userJoinedWithSocket(this, socket);
    },
    
    leftEventWithSocket: function(event, socket) {
        logger.debug("leftEventWithSocket: " + event.id + " s.id: " + socket.id);
        ...
        if(events[event.id].length==0) {
            ...
            // Only mark the user as left if there's no sockets left for this
            // user in this event.
            event.userLeft(this);
        }
...
server_model.ServerEvent = model.Event.extend({
    ...
    userJoinedWithSocket: function(user) {
        // Join this user's sockets to the event's channels
        
        if(this.get("users").include(user)) {
            // The user is already in this event, so we need to do things a
            // little differently. This is basically someone opening another
            // window on the same event. The trick is we need to publish
            // the posts to just that socket, since we don't want to double-
            // publish to the other socket for this user.
            var promotedPostsByTime = this.get("posts").getPromotedPosts({sort:"time"});
            _.each(promotedPostsByTime.reverse(), function(post) {
                post.publishToSocket(user);
            });
        } else {
    ...
    fetch: function() {
        // we're going to use fetch as our hook to rebuild this event
        // from its journal. 
        actions.runJournaledActionsForEvent(this);
    },
