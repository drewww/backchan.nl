...
server_model.ServerPost = model.Post.extend({
    ...
    addVote: function(fromUserId) {
        var result = model.Post.prototype.addVote.call(this, fromUserId);
    ...
    promoted: function() {
        ...
        if(this.previous("promotedAt")==null && this.get("promotedAt")!=null){
            ...
            logger.warn("Promoted event triggered in a weird situation.");
        }
    },
...
server_model.ServerEvent = model.Event.extend({
    ...
    defaults: function() {
        ...
        // For now I'm hardcoding dispatchers at the Event level. I don't
        // forsee lauching with flexible dispatchers. These can always
        // be overridden with an init parameter. This is mostly for testing.
        defaults["dispatch"] = new dispatch.BaseDispatcher(this);
    ...
    postVotedOn: function(post, voterId) {
        ...
        if(_.isUndefined(voter) || _.isNull(voter)) {
            logger.warn("Received a notice of a vote from a user who is not connected to the event.");
            return;
        }
