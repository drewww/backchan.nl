...
server_model.Journaler.prototype = {
    ...
    handleNewModel: function(newModel) {
        if(!this.running) return;
        
        newModel.bind("change", this.handleModelChange, this);
        
        this.writeAttributes("CREATE", newModel.attributes);
    },
    
    handleModelChange: function(changedModel) {
        if(!this.running) return;
        
        this.writeAttributes("CHANGE", newModel.changedAttributes)
    },
    
    writeAttributes: function(entryType, setAttributes) {
        // entryType is CREATE or CHANGE
        
        var entry = {timestamp:new Date().getTimestamp(), type:entryType, 
